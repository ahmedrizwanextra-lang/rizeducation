        ...
        {menuOpen && (
          <div className="border-t bg-white md:hidden">
            <ul className="flex flex-col gap-2 px-6 py-4">
              ...
              <li><button onClick={() => setModalOpen(true)} className="mt-2 w-full rounded-lg bg-[#f0b429] px-6 py-3 text-sm font-semibold text-[#1a2744]">Start Application Process</button></li>
            </ul>
          </div>
        )}
      </header>
      ...
      <section className="bg-[#1a2744] px-6 py-16">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-10 md:flex-row">
          <div className="max-w-xl text-white">
            ...
            <button onClick={() => setModalOpen(true)} className="mt-6 inline-flex items-center gap-2 rounded-lg bg-[#f0b429] px-8 py-4 text-base font-bold text-[#1a2744] transition hover:bg-[#d9a020]">Start Application Process <i className="fas fa-arrow-right"></i></button>
          </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="relative w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
            <button onClick={closeModal} className="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-full bg-gray-100 text-gray-600 transition hover:bg-gray-200"><i className="fas fa-times"></i></button>
            <div className="h-1.5 w-full overflow-hidden rounded-t-2xl bg-gray-100"><div className="h-full bg-[#f0b429] transition-all duration-500" style={{ width: `${(step / 5) * 100}%` }}></div></div>
            <div className="p-8 md:p-10">
              <div className="mb-6 text-sm font-semibold text-[#f0b429]">Step {step} of 5</div>
              ...
              <div className="mt-8 flex justify-between">
                ...
                <button onClick={nextStep} disabled={!canProceed()} className="rounded-lg bg-[#1a2744] px-6 py-3 text-sm font-semibold text-white transition hover:bg-[#141e36] disabled:cursor-not-allowed disabled:opacity-40">{step === 5 ? "Continue to Account" : "Next"} <i className="fas fa-arrow-right ml-2"></i></button>
              </div>
            </div>
          </div>
        </div>
