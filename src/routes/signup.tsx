...
export const Route = createFileRoute("/signup")({
  head: () => ({
    meta: [
      { title: "Create Account — Pathway Education Counselling" },
    ],
  }),
  component: Signup,
});
...
function Signup() {
  ...
  const qualData = (() => {
    try {
      return JSON.parse(localStorage.getItem("qualificationData") || "null");
    } catch {
      return null;
    }
  })();
        ...
        <div className="grid gap-8 md:grid-cols-[1fr_1.3fr]">
          ...
          {qualData && (
            <div className="rounded-2xl bg-[#1a2744] p-8 text-white shadow-xl">
              <div className="mb-6 text-sm font-bold uppercase tracking-widest text-[#f0b429]">
                Your Application
              </div>
              ...
              <div className="space-y-4 text-sm">
                <div>
                  <div className="text-xs uppercase tracking-wider text-gray-400">Degree</div>
                  <div className="font-semibold">{qualData.degree}</div>
                </div>
                <div>
                  <div className="text-xs uppercase tracking-wider text-gray-400">Current Level</div>
                  <div className="font-semibold">{qualData.education}</div>
                </div>
                <div>
                  <div className="text-xs uppercase tracking-wider text-gray-400">Countries</div>
                  <div className="font-semibold">{qualData.countries?.join(", ")}</div>
                </div>
                <div>
                  <div className="text-xs uppercase tracking-wider text-gray-400">Budget</div>
                  <div className="font-semibold">
                    {qualData.currency === "USD" ? "$" : "₨"}
                    {qualData.budget?.toLocaleString()}
                  </div>
                </div>
                <div>
                  <div className="text-xs uppercase tracking-wider text-gray-400">Institution</div>
                  <div className="font-semibold">{qualData.institution}</div>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="rounded-2xl bg-white p-8 shadow-xl md:p-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <button
                type="submit"
                className="w-full rounded-lg bg-[#f0b429] px-6 py-4 text-base font-bold text-[#1a2744] transition hover:bg-[#d9a020]"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white p-10 text-center shadow-2xl">
            ...
            <p className="mt-4 text-gray-600 leading-relaxed">
              Your account has been created and application details have been submitted.
            </p>
            <p className="mt-2 text-sm text-gray-500">
              We'll contact you at <strong>{email}</strong> shortly.
            </p>
            <a
              href="/"
              className="mt-8 inline-block rounded-lg bg-[#1a2744] px-8 py-3 text-base font-semibold text-white transition hover:bg-[#141e36]"
            >
              Back to Home
            </a>
          </div>
