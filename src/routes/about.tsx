...
const missionValues = [
  ...
  { icon: "handshake", title: "Our Commitment", desc: "We are committed to honest advice, transparent processes and student success at every step." },
  { icon: "star", title: "Our Values", desc: "Integrity, excellence and empathy are at the core of everything we do." },
  { icon: "users", title: "Student First", desc: "Every decision we make is centered around what's best for our students and their future." },
];
...
const counsellors = [
  { name: "Ayesha Malik", role: "Lead Counsellor", desc: "6+ years of experience in international education and career guidance. Specializes in UK & Canada admissions.", img: "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=300&h=300&fit=crop&crop=face" },
  { name: "Hassan Ali", role: "Senior Counsellor", desc: "Expert in university placements and application strategies. Specializes in Australia & NZ.", img: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=300&h=300&fit=crop&crop=face" },
    ...
    <div className="min-h-screen bg-white font-sans text-[#1a2744]" style={{ fontFamily: "'Montserrat', sans-serif" }}>
      ...
      <section className="px-6 py-16 md:py-20">
        <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-12 md:grid-cols-2">
          <div>
            <div className="text-sm font-bold uppercase tracking-widest text-[#f0b429]">About Us</div>
            <h1 className="mt-3 text-5xl font-black leading-tight text-[#1a2744] md:text-6xl">We're here to guide you towards a brighter future.</h1>
            <div className="my-5 h-1 w-12 rounded bg-[#f0b429]"></div>
            <p className="max-w-lg text-gray-600 leading-relaxed">Pathway Education Counselling was founded with a simple mission: to help O/A Level students in Pakistan unlock global opportunities through the right guidance, support and resources.</p>
            <div className="mt-8 inline-flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 px-5 py-4">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#1a2744]"><i className="fas fa-user-friends text-[#f0b429]"></i></div>
              <span className="text-sm font-semibold text-[#1a2744]">Trusted by 500+ students across Pakistan</span>
            </div>
          </div>
          <div><img src="https://images.unsplash.com/photo-1573497019940-1c28c88b4f3e?w=700&h=500&fit=crop" alt="Counsellor" className="h-full w-full rounded-2xl object-cover shadow-lg" /></div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {missionValues.map((item) => (
              <div key={item.title} className="rounded-xl border border-gray-200 bg-white p-8 text-center transition hover:-translate-y-1 hover:shadow-lg">
                <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-full bg-[#1a2744]"><i className={`fas fa-${item.icon} text-xl text-[#f0b429]`}></i></div>
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-2 gap-8 md:grid-cols-4">
          {stats.map((stat) => (
            <div key={stat.label} className="flex items-center gap-4">
              ...
              <div><div className="text-3xl font-black">{stat.value}</div><div className="text-sm opacity-80">{stat.label}</div></div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-6 py-20">
        <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-12 md:grid-cols-2">
          <div className="relative">
            <img src="https://images.unsplash.com/photo-1497366216548-37526070297c?w=700&h=500&fit=crop" alt="Workspace" className="h-full w-full rounded-2xl object-cover shadow-lg" />
            <div className="absolute inset-0 flex items-center justify-center rounded-2xl bg-[#1a2744]/70">
              <div className="px-8 text-center text-white"><div className="text-2xl font-bold md:text-3xl leading-tight">Opportunities don't happen.<br />You create them.</div></div>
            </div>
          </div>
          <div>
            <h2 className="text-4xl font-extrabold text-[#1a2744]">Our Story</h2>
            <div className="my-4 h-1 w-12 rounded bg-[#f0b429]"></div>
            <p className="mt-6 text-gray-600 leading-relaxed">We started Pathway Education Counselling with a belief that every student deserves access to the world's best opportunities—regardless of their background or where they come from.</p>
            <p className="mt-4 text-gray-600 leading-relaxed">Having seen countless students struggle with confusing information, unclear processes and lack of personalized support, we knew there had to be a better way.</p>
            <p className="mt-4 text-gray-600 leading-relaxed">Today, we are proud to be one of Pakistan's trusted counselling platforms, helping O/A Level students build successful futures at top universities around the globe.</p>
            <div className="mt-8"><div className="font-serif italic text-xl text-[#1a2744]">— Founder</div><div className="mt-1 text-sm font-bold text-[#1a2744]">Founder & Lead Counsellor</div><div className="text-sm text-gray-500">Pathway Education Counselling</div></div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-gray-50 px-6 py-20 text-center">
        <div className="mx-auto max-w-7xl">
          ...
          <p className="mt-3 text-gray-600">Experienced. Certified. Passionate about your success.</p>
          <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {counsellors.map((c) => (
              <div key={c.name} className="rounded-xl border border-gray-200 bg-white p-6 text-left transition hover:-translate-y-1 hover:shadow-lg">
                <img src={c.img} alt={c.name} className="mx-auto mb-4 h-24 w-24 rounded-full object-cover" />
                ...
                <a href="#" className="mt-4 inline-flex h-8 w-8 items-center justify-center rounded bg-[#1a2744] text-white transition hover:bg-[#0ea5e9]"><i className="fab fa-linkedin-in text-sm"></i></a>
              </div>
            ))}
          </div>
        </div>
